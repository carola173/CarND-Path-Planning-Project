...
int main() {
  ...
  // Reference velocity.
  double ref_velocity = 0.0; // this value is in  "mph" unit
  
  
  h.onMessage([&ref_velocity ,&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
    ...
    //cout << sdata << endl;
	//cout<<"enter onMessage";
	 // Starting from the middle of the lane - Our Car lane
	int lane = 1;
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			// to get the lane number for the Main car
			if(car_d <= 4.0)
				lane=0;
			else if(car_d <= 8.0)
				lane=1;
			...
			//Storing the previous path point size
			int previous_path_size=previous_path_x.size();
			// To avoid the collision , we would store the car s coordinates to end_path_s value when the previous_path_size is greater than 0 
			// so that we start from the end point for the next iteration thus avoiding any collision
			...
			// Declaring the variables to get the prediction for the other cars under the vicinity of our car
			bool front_car=false;
			bool left_car=false;
			bool right_car=false;
			// Reading all the reading from the sensor fusion
			...
			// Steps for what has to be done :- Behavior
			...
			if(front_car){
				...
				// Check the left lane if there is no car then turn left
				if (!left_car && lane > 0) // Show there is no car in left and left lane is also present to turn
					lane --;
				else if (!right_car && lane !=2) // show there is not car in right and right lane is also present to turn
					lane ++;
				else
				...
				if(lane!=1){
					if ( ( lane == 0 && !right_car ) || ( lane == 2 && !left_car ) ) 
						lane = 1; // Back to center position.
				}
			...
			vector<double> ptsx;
            vector<double> ptsy;
			cout<< "Speed difference : " <<speed_difference << endl;
			
            double ref_x = car_x;
			//cout<<"  x_value" << ref_x;
            double ref_y = car_y;
			//cout<<"y_value" << ref_y;
			// Since the simulator data for yaw is in degrees thus converting it to radians values
            double ref_yaw = deg2rad(car_yaw);
			//cout<<"ref_yaw"<< ref_yaw;
			//cout << "previous_path_size : " <<previous_path_size;
			if(previous_path_size < 2){
				double prev_carx=car_x - cos(car_yaw);
				double prev_cary=car_y - sin(car_yaw);
				// Inserting the x coordinates 
                ptsx.push_back(prev_carx);
                ptsx.push_back(car_x);
				// Inserting the y coordinates 
			...
			// Adding up target points in the future usage.
            ...
            // Adjusting to the local car coordinates
            ...
            tk::spline s;
			for(int i = 0; i<ptsx.size(); i++){
				cout<<"X:"<<ptsx[i]<<"Y:"<<ptsy[i];
			}
            s.set_points(ptsx, ptsy);
			
			// Adding the output points from the previous path for continuity
			for ( int i = 0; i < previous_path_x.size(); i++ ) {
              next_x_vals.push_back(previous_path_x[i]);
              next_y_vals.push_back(previous_path_y[i]);
            }
			// To calculate distance y position .
